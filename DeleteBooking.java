...
public class DeleteBooking extends Frame implements ActionListener, WindowListener{
	...
	public void actionPerformed(ActionEvent e){
		...
		if(sig.equals("Delete")){
			if(isEmpty(t) || isEmpty(t3)){
				...
				if(!checkAdminAuth()){
					try{
					String sql="DELETE FROM booking WHERE booking.booking_id = '"+t.getText()+"' && c_id = (select c_id from customer where password = '"+t3.getText()+"')";
					...
					try{
					String sql2="DELETE FROM booking WHERE booking.booking_id = '"+t.getText()+"'";
